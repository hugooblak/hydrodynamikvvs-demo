<?php
/**
 * Hydro Dynamik VVS AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Hydro Dynamik VVS AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through hy_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'hydrodynamikvvs', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'hydrodynamikvvs' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'hydrodynamikvvs' ) ) );

		register_block_pattern_category( 'hydrodynamikvvs-sections', array( 'label' => __( 'Hydro Dynamik VVS AB sections', 'hydrodynamikvvs' ) ) );
		register_block_pattern_category( 'hydrodynamikvvs-pages', array( 'label' => __( 'Hydro Dynamik VVS AB page layouts', 'hydrodynamikvvs' ) ) );
	}
);
