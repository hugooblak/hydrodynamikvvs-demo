 ...
 * or photo. They call hy_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Hydro Dynamik VVS AB.
 *
 ...
 * @return string
 */
function hy_sample_note() {
	return hy_lead( 'ar_riktig' ) ? '' : ' <span class="hy-sample">(platshållare)</span>';
}
 ...
 * @return string File name for hy_lead_img().
 */
function hy_lead_bild( $i ) {
	$bilder = (array) hy_lead( 'bilder', array() );
 ...
 * @return string
 */
function hy_ring_label() {
	$namn = hy_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, hy_lead( 'telefon' ) ) : 'Ring ' . hy_lead( 'telefon' );
}
