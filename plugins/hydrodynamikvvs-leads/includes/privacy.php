...
add_filter(
	...
	function ( $exporters ) {
		$exporters['hydrodynamikvvs-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'hydrodynamikvvs-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'hydrodynamikvvs-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'hydrodynamikvvs-leads',
						'group_label' => __( 'Quote requests', 'hydrodynamikvvs-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['hydrodynamikvvs-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'hydrodynamikvvs-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
