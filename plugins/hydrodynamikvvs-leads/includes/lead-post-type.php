...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'hydrodynamikvvs-leads' ),
					'singular_name' => __( 'Lead', 'hydrodynamikvvs-leads' ),
					'edit_item'     => __( 'Förfrågan', 'hydrodynamikvvs-leads' ),
					'search_items'  => __( 'Search leads', 'hydrodynamikvvs-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'hydrodynamikvvs-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'hydrodynamikvvs-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'hydrodynamikvvs-leads' ),
			'npl_phone'    => __( 'Phone', 'hydrodynamikvvs-leads' ),
			'npl_service'  => __( 'Needs', 'hydrodynamikvvs-leads' ),
			'npl_timeline' => __( 'When', 'hydrodynamikvvs-leads' ),
			'npl_zip'      => __( 'ZIP', 'hydrodynamikvvs-leads' ),
			'npl_source'   => __( 'Source', 'hydrodynamikvvs-leads' ),
			'npl_status'   => __( 'Status', 'hydrodynamikvvs-leads' ),
			'date'         => __( 'Received', 'hydrodynamikvvs-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'hydrodynamikvvs-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'hydrodynamikvvs-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'hydrodynamikvvs-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'hydrodynamikvvs-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'hydrodynamikvvs-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'hydrodynamikvvs-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'hydrodynamikvvs-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'hydrodynamikvvs-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'hydrodynamikvvs-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'hydrodynamikvvs-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'hydrodynamikvvs-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'hydrodynamikvvs-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
